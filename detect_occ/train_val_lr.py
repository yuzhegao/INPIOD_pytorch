...
def main():
    ...
    criterion = [criterion_edge, criterion_ins]
...
def train_epoch(train_loader, model, criterion, optimizer, epoch, train_writer, output_writers, config, logger):
    ...
    for batch_idx, (inputs, sem_anno, ins_anno, n_objects, img_path) in enumerate(train_loader):
        ...
        if batch_idx % config.default.frequent == 0:
            ...
            train_info += 'TotalLoss {}\t Loss_edge {:.5f}\t Loss_ins {:.5f}\t '.format(
                total_loss, edge_loss, ins_loss)
...
def val_epoch(val_loader, model, criterion, epoch, output_writers, logger, res_path, config, isTest=False):
    ...
    out_path_ins = os.path.join(res_path, 'result_vis_KMeans')
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, sem_anno, ins_anno, n_objects, img_path) in enumerate(val_loader):
            ...
            if save_cluster:
                print("=>clustering......")
                fg_seg_pred, ins_seg_pred, n_objects_pred = cluster(edgemap, insmap, n_objects)
                ...
                fg_seg_pred_pil.save(os.path.join(out_path_ins, img_id + '-fg_mask.png'))
...
def cluster(sem_seg_prediction, ins_seg_prediction, n_objects_prediction):
    ...
    :param ins_seg_prediction: [N, H, W]
    :return:
    """
    seg_height, seg_width = ins_seg_prediction.shape[1:]

    sem_seg_prediction = (sem_seg_prediction > 0.5).astype(np.uint8)
