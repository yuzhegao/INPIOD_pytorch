...
class ResUnetModel(nn.Module):
    def __init__(self, config, encoder_arch='resnet50', pretrained=True):
        ...
        self.out0_ins = OutputBlock_ins(64, 32)
    ...
    def forward(self, input0):
        ...
        out0_ins = self.out0_ins(deconv0)
        # out0_ins = self.out0_ins(conv4)

        return out0_edge, out0_ins
...
class OutputBlock_ins(nn.Module):
    def __init__(self, input_nc, output_nc=32, use_bias=True):
        super(OutputBlock_ins, self).__init__()
        self.model = nn.Sequential(
            nn.Conv2d(input_nc, output_nc, kernel_size=3, padding=1, stride=1, bias=use_bias),
            nn.BatchNorm2d(output_nc),
            nn.ReLU(inplace=True),

            # nn.Conv2d(output_nc, output_nc, kernel_size=3, padding=1, stride=1, bias=use_bias),
            # nn.BatchNorm2d(output_nc),
            # nn.ReLU(inplace=True),
            nn.Conv2d(output_nc, output_nc, padding=0, stride=1, kernel_size=1, bias=use_bias),
            # nn.Conv2d(output_nc, output_nc, padding=0, stride=1, kernel_size=1, bias=use_bias)
        )
        # self.model = nn.Sequential(
        #     nn.Conv2d(512, 256, kernel_size=1, padding=0, stride=1, bias=use_bias),
        ...
        #                        groups=32, bias=False),
        #     nn.Conv2d(32, 32, kernel_size=1, padding=0, stride=1, bias=use_bias)
        # )
...
class BCP(nn.Module):
    ...
    def __init__(self, input_nc=2048, output_nc=256):
        ...
        self.onePoneConV = nn.Conv2d(output_nc, output_nc, kernel_size=1, stride=1,
                                     padding=0)  # lr_mult & bias_term & bias_filler ??????

        self.W = nn.Sequential(
            nn.Conv2d(output_nc, output_nc, kernel_size=1, stride=1, padding=0),
            # lr_mult & bias_term & bias_filler ??????
            nn.BatchNorm2d(output_nc)
        )
